...
public class ActionSheetDialog {
        ...
        for (int i = 1; i <= size; i++) {
            ...
            if (size == 1) {
                if (showTitle) {
                    ...
                    textView.setBackgroundResource(R.drawable.actionsheet_alone_selector);
                }
            } else {
                if (showTitle) {
                    if (i >= 1 && i < size) {
     ...
     * @return
     */
    private View getSegmentingLine(View view, int height) {
        view.setBackgroundColor(ContextCompat.getColor(context, R.color.normal_gray));
